...
void update_run_time(thread_info_t *info) {
	...
	long temp = time_difference(&info->suspend_time, &info->resume_time);
...
void update_wait_time(thread_info_t *info) {
	...
	long temp = time_difference(&info->resume_time, &info->suspend_time);
 ...
 static void *scheduler_run(void *unused)
{
	...
	in.it_value.tv_sec=QUANTUM;
	in.it_interval.tv_sec=QUANTUM;
